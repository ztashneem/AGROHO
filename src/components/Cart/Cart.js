...
const Cart = (props) => {
    ...
    return (
        <div className='cart'>
            ...
            < >{carts.map(products => <h5 key={products.id} className='bg-infos p-lg-3'>{products.name}</h5>)}</>
            <h4 className='p-lg-3  price'>Total Price: $ {carts.length ? carts.reduce((a, b) => ({ price: a.price + b.price })).price : 0}</h4>
            {
                carts.length ? (
                    <>
